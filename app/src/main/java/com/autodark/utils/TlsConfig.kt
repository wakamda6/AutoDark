...
object TlsConfig {
    ...
    // 当前连接模式，默认无加密
    var mode: String
        get() = SaveKeyValues.getValue(KEY_MODE, MODE_NONE) as String
        set(value) = SaveKeyValues.putValue(KEY_MODE, value)

    // 进入当前模式前的上一个模式（用于退出双向认证时回退）
    var previousMode: String
        get() = SaveKeyValues.getValue(KEY_PREV_MODE, MODE_NONE) as String
        set(value) = SaveKeyValues.putValue(KEY_PREV_MODE, value)
